...
function findWakeupFile() {
  const files = fs.readdirSync(inputDir);
  const wakeupFile = files.find((file) => file.endsWith(".wakeup_schedule"));
  if (!wakeupFile)
    throw new Error(
      "未找到 .wakeup_schedule 文件，请确保将文件放在 input 文件夹内",
    );
  return path.join(inputDir, wakeupFile);
}
const wakeupRaw = fs.readFileSync(findWakeupFile(), "utf-8");
...
console.log(
  fs
    .readFileSync(scriptSrc, "utf-8")
    .replace(/\/\*\*[\s\S]*?\*\/|\/\/.*/g, "")
    .replace(/\r?\n\s+/g, "")
    .replace("__DATA__", JSON.stringify(parseWakeup())),
);
