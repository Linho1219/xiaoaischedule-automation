(async (data) => {
  ...
  async function dismissDialog(buttonText) {
    clickEl(
      await select(
  ...
  function select(selector, timeout = 5000, interval = 100) {
  ...
  async function selectPicker(picker, value) {
  ...
  async function applyClass() {
    ...
    for (const entry of data.classList) {
      ...
      const emptyBox = [
        ...
        return (
          boxRect.x > dayBoxRect.x &&
          boxRect.x < dayBoxRect.x + dayBoxRect.width
        );
      });
      clickEl(emptyBox);
      ...
      const weekTypeItem = await select(
        `[aria-label^="${weekTypeArialLabel}"]`,
      );
